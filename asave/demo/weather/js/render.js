...
// 采用window作为参数传入，将功能函数挂载到window上
  let WaterPolo = function(selector, userOptions) {
    'user strict'
    ...
    let mergeOption = function(userOptions, options) {
      Object.keys(userOptions).forEach(function(key) {
        options[key] = userOptions[key]
      })
    }
    // 生成液面
    let makeLiquaid = function(ctx, xOffset, waveWidth, waveHeight, color) {
      ctx.save()
      let points = []// 用于存放绘制Sin曲线的点
      ctx.beginPath()
      // 在x轴上取点
      for (let x = 0; x < options.cW; x += 20 / options.cW) {
        // 此处坐标(x,y)的取点，依靠公式 “振幅高*sin(x*振幅宽 + 振幅偏移量)”
        let y = -Math.sin(x * waveWidth + xOffset)
        // 液面高度百分比改变
        let dY = options.cH * (1 - options.nowRange / 100)
    ...
    let init = function() {
      ...
      // 圆属性
      let r = options.cH / 2 // 圆心
      let cR = r - 6 // 圆半径 决定圆的大小
      ...
      drawCircle(ctx)(function drawFrame() {
