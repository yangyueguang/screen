...
(function(global) {
  ...
  let miniumFontSize = (function getMiniumFontSize() {
    if (!isSupported) {
      return
    }
  ...
  let WordCloud = function WordCloud(elements, options) {
    ...
    function random_hsl_color() {
      let aColor = [[46, 199, 201], [182, 162, 222], [90, 177, 239], [255, 165, 138], [255, 203, 95], [216, 122, 128], [255, 99, 71], [186, 85, 211], [255, 127, 80], [255, 165, 0]]
      return 'rgb(' + aColor[parseInt(Math.random() * (aColor.length - 1))][0] + ',' + aColor[parseInt(Math.random() * (aColor.length - 1))][1] + ',' + aColor[parseInt(Math.random() * (aColor.length - 1))][2] + ')'
    }
    switch (settings.color) {
      case 'random-dark':
        getTextColor = function getRandomDarkColor() {
          return random_hsl_color()
        }
        break
      case 'random-light':
    ...
    let infoGrid = []
    let hovered
    let getInfoGridFromMouseEvent = function getInfoGridFromMouseEvent(evt) {
      let canvas = evt.currentTarget
      let rect = canvas.getBoundingClientRect()
      let eventX = evt.clientX - rect.left
      let eventY = evt.clientY - rect.top
    ...
    let wordcloudhover = function wordcloudhover(evt) {
      let info = getInfoGridFromMouseEvent(evt)
      if (hovered === info) {
        return
      }
      hovered = info
