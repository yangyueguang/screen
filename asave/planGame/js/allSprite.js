(function(W){
	...
	W.planBehavior = [
		...
		shot:function(sprite){
			...
			let missleAngle = 0.1
			for(let i=1;i<sprite.fireLevel;i++){
	...
	W.starPainter = {
		...
		cache:function(sprite){
			...
			let opacity = 0.5,addopa = 0.8/sprite.lightLength;
			for(let i=1;i<=sprite.lightLength;i+=2){
				opacity-=addopa;
	...
	W.badPlanBehavior = [{
		execute:function(sprite,time){
			...
			if(sprite.top>0){
				let num = sprite.badKind===1?0.002:0.01;
				let random = Math.random()
				if(random<num){this.shot(sprite)}
			}
			sprite.top += sprite.speed;
